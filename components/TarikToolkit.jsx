...
export default function TarikToolkit() {
  ...
  const runScrape = async () => {
    ...
    try {
      ...
      const { text, error } = await proxyRes.json();
  ...
  const searchSkills = async () => {
    ...
    setLoading(true); setSkillsResults([]);
    try {
      ...
      while ((match = skillRegex.exec(html)) && results.length < 10) {
        results.push({ name: match[1].trim(), desc: match[2].trim(), url: `https://mcpmarket.com/search?q=${encodeURIComponent(skillsQuery)}` });
      }
      if (results.length === 0) {
        results.push({ name: "MCPMarket", desc: "Voir les résultats sur mcpmarket.com", url: `https://mcpmarket.com/search?q=${encodeURIComponent(skillsQuery)}` });
      }
      setSkillsResults(results);
    } catch (err) {
      setSkillsResults([{ name: "❌ Erreur", desc: err.message, url: `https://mcpmarket.com/search?q=${encodeURIComponent(skillsQuery)}` }]);
    }
    setLoading(false);
  };
    ...
    <div style={s}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #222", background: "#080808" }}>
        {["scrape","security","skills"].map(t => (
          <button key={t} onClick={() => { setTab(t); setResult(""); setSkillsResults([]); }} style={{
